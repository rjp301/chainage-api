...
class Topcon:
    ...
    def volume_calc(self):
        for index, row in self.data_pts.iterrows():
            ...
            try:
                depth = pt.z - self.ditch.elevation_at_pt(pt)
            except Exception as e:
                continue

            width_top = (
                depth / self.slope * 2 + self.width_bot
                if self.slope > 0
                else self.width_bot
            )
            ...
            self.data_pts.at[index, "area"] = (
                self.width_bot * depth + (width_top - self.width_bot) / 2 * depth
            )
        ...
        self.data_rng["area_avg"] = (
            self.data_rng["area_beg"] + self.data_rng["area_end"]
        ) / 2
